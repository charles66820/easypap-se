...
void pixelize_init_ocl(void)
{
  if (GPU_TILE_W != GPU_TILE_H)
    exit_with_error("Tiles should have a square shape (%d != %d)", GPU_TILE_W, GPU_TILE_H);

  if (GPU_TILE_W < PIX_BLOC || (GPU_TILE_W % PIX_BLOC != 0))
    exit_with_error("Tile size (%d) must be a multiple of PIX_BLOC (%d)", GPU_TILE_W, PIX_BLOC);
}
