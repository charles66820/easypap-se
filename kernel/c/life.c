...
void life_init(void)
{
  ...
  if (_table == NULL)
  {
    ...
    _table = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

    _alternate_table = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  }
}
...
unsigned life_compute_tiled(unsigned nb_iter)
{
  ...
  for (unsigned it = 1; it <= nb_iter; it++)
  {
    ...
    if (!change)
    { // we stop if all cells are stable
      res = it;
      break;
    }
  }
...
static void inline life_rle_parse(char *filename, int x, int y, int orientation)
{
  rle_lexer_parse(filename, x, y, set_cell, orientation);
}

static void inline life_rle_generate(char *filename, int x, int y, int width, int height)
{
...
static void otca_autoswitch(char *name, int x, int y)
{
  ...
  life_rle_parse("data/rle/autoswitch-ctrl.rle", x + 123, y + 1396, RLE_ORIENTATION_NORMAL);
}

static void otca_life(char *name, int x, int y)
{
  life_rle_parse(name, x, y, RLE_ORIENTATION_NORMAL);
  life_rle_parse("data/rle/b3-s23-ctrl.rle", x + 123, y + 1396, RLE_ORIENTATION_NORMAL);
}
...
static void at_the_four_corners(char *filename, int distance)
{
  ...
  life_rle_parse(filename, distance, distance, RLE_ORIENTATION_HINVERT | RLE_ORIENTATION_VINVERT);
}
...
void life_draw_meta3x3(void)
{
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      otca_life(j == 1 ? "data/rle/otca-on.rle" : "data/rle/otca-off.rle", 1 + j * (2058 - 10), 1 + i * (2058 - 10));
}
...
void life_draw_ship(void)
{
  ...
  {
    life_rle_parse("data/rle/greyship.rle", DIM - 100, y, RLE_ORIENTATION_NORMAL);
  }
}
...
{
  life_rle_parse("data/rle/clown-seed.rle", DIM / 2, DIM / 2, RLE_ORIENTATION_NORMAL);
}
